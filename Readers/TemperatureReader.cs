...
namespace MapGeneration.Readers
{
    public class TemperatureReader
    {
        ...
        public void readTemperaturesTxt()
        {
            string filePath = folderPath + "\\TemperatureChart.txt";
            foreach (string line in File.ReadAllLines(filePath))
            {
                ...
                if (parts[0].Equals("MinInf"))
                    ...
                    lowerLimit = (float)double.Parse(parts[0]);
                Color color = ColorTranslator.FromHtml(parts[2]);
                temperatures.Add(lowerLimit, new TemperatureObject(parts[1], color));
            }
        }

        public void readTemperaturesXlsx()
        {
            string filePath = folderPath + "\\TemperatureChart.xlsx";
